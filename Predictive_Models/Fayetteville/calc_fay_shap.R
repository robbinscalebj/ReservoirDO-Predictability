#Calculate SHAP vals for Fayetteville test set predictions at top middle and bottom depths 
#####
...
here::i_am("Predictive_Models/Fayetteville/calc_fay_shap.R")

fay_randfor<-readRDS(here("Predictive_Models/Fayetteville/randfor_fit.Rds")) #randfor_fit  is not specifically a workflow - this has already had last_fit() run on it because we had to specify the exact lag6 recipe for the final fit (instead of the split object) because ranger can't handle missing values
...
prep_data <- function(dat){
  ...
  shap_dat_test <- shap_prep|>bake(dat|>drop_na()|>filter(validation_period == "testing")|>
                                     filter(Depth_m %in% c(0,5,8.5))|>ungroup())
  dat_list <- list("shap_dat_train" = shap_dat_train,"shap_dat_test" = shap_dat_test)
}



train_vec <- prep_data(fay_data)|>pluck("shap_dat_train")
test_vec <- prep_data(fay_data)|>pluck("shap_dat_test")
...
nested_test_data <- test_vec|>
  mutate(Depth_m_raw = as_factor(round(Depth_m,2)),
         Depth_m_raw = fct_recode(Depth_m_raw, `0` = "-1.63",`5` = "0.3",`8.5` = "1.65"))|> #convert normalized depths to new grouping variable of actual depths
...
#set up model
model <- fay_randfor|>extract_fit_parsnip()|>pluck(3)
...
# Loop
fay_shaps <- foreach(i = 1:nrow(nested_test_data), .combine = 'c', .options.snow = opts, 
...
shap_names <- names(fay_shaps)
fay_shaps_df <- fay_shaps|>map(~flatten(.)|>as_tibble())|>
...
fay_shaps_df|>write_csv(here("Predictive_Models/Fayetteville/SHAP_vals.csv"))
  
  fay_shap_sums <- fay_shaps_df|>
...
#ggplot(aes(x = mean_SHAP, y = variable, fill = Depth_m_raw))+
  ggplot(fay_shap_sums|>filter(Depth_m_raw == 0), aes(y=reorder_within(variable, mean_SHAP, list(val_period,Depth_m_raw)), x=mean_SHAP))+
