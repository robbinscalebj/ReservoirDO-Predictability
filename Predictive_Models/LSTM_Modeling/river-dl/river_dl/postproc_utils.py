...
def fmt_preds_obs(pred_data,
    ...
    if obs_file.endswith("zarr"):
        obs = xr.open_zarr(obs_file,consolidated=False).to_dataframe()
    elif obs_file.endswith("feather"):
        obs = pd.read_feather(obs_file)
        obs = obs.set_index([time_idx_name, spatial_idx_name])
    elif obs_file.endswith("csv"):
        obs = pd.read_csv(obs_file, parse_dates=[time_idx_name])
        obs = obs.set_index([time_idx_name, spatial_idx_name])
