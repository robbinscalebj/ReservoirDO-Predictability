#Calculate SHAP vals for Maumelle test set predictions at top middle and bottom depths 
#####
...
here::i_am("Predictive_Models/Maumelle/calc_maum_shap.R")
...
train_vec <- prep_data(maum_data)|>pluck("shap_dat_train")
test_vec <- prep_data(maum_data)|>pluck("shap_dat_test")
...
nested_test_data <- test_vec|>
  mutate(Depth_m_raw = as_factor(round(Depth_m,2)),
         Depth_m_raw = fct_recode(Depth_m_raw, `0` = "-1.61",`5` = "0.16",`9` = "1.58"))|> #convert normalized depths to new grouping variable of actual depths
...
#set up model
model <- maum_randfor|>extract_fit_parsnip()|>pluck(3)
...
# Loop
maum_shaps <- foreach(i = 1:nrow(nested_test_data), .combine = 'c', .options.snow = opts, 
...
shap_names <- names(maum_shaps)
maum_shaps_df <- maum_shaps|>map(~flatten(.)|>as_tibble())|>
...
maum_shaps_df|>write_csv(here("Predictive_Models/Maumelle/SHAP_vals.csv"))
  
  maum_shap_sums <- maum_shaps_df|>
...
#ggplot(aes(x = mean_SHAP, y = variable, fill = Depth_m_raw))+
  ggplot(maum_shap_sums|>filter(Depth_m_raw == 0), aes(y=reorder_within(variable, mean_SHAP, list(val_period,Depth_m_raw)), x=mean_SHAP))+
