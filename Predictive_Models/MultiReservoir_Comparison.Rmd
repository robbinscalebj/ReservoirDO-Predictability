...
seas_shap.p3<-ggplot()+
  geom_jitter(data = agg_shap2|>filter(variable %in% focal_vars)|>mutate(variable = fct_recode(variable, Wind = "Wind_S&W", `Water Temp` = "WaterTemp_C", Depth = "Depth_m")), aes(x = season, y = SHAP,  shape = lake,fill = lake), alpha = 0.2, position = position_dodge(width = 0.65), size = 3)+
  geom_point(data = sm1_em_tidy|>filter(variable %in% focal_vars)|>mutate(variable = fct_recode(variable, Wind = "Wind_S&W", `Water Temp` = "WaterTemp_C", Depth = "Depth_m")), aes(x = season, y = estimate,  shape = lake, fill = lake, group = lake), size = 5, alpha = 1, position = position_dodge(width = 0.65))+
  geom_errorbar(data = sm1_em_tidy|>filter(variable %in% focal_vars)|>mutate(variable = fct_recode(variable, Wind = "Wind_S&W", `Water Temp` = "WaterTemp_C", Depth = "Depth_m")),aes(x = season, ymin = conf.low, ymax = conf.high, group = lake), color = "black",   width = 0.4, position = position_dodge(width = 0.65))+
