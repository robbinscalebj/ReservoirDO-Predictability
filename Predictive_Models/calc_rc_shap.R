#Calculate SHAP vals for Richland-Chambers test set predictions at top middle and bottom depths 
#####
...
here::i_am("Predictive_Models/calc_rc_shap.R")

rc_randfor<-readRDS(here("Predictive_Models/trained_workflows/randfor_fit.Rds")) 
rc_data <- rc_randfor[[1]][[1]][["data"]]|>
  mutate(Datetime = round_date(Datetime, "2 hours"))
...
train_vec <- prep_data(rc_data)|>pluck("shap_dat_train")
test_vec <- prep_data(rc_data)|>pluck("shap_dat_test")
...
#set up model
model <- rc_randfor|>extract_fit_parsnip()|>pluck(3)
...
# Loop
rc_shaps <- foreach(i = 1:nrow(nested_test_data), .combine = 'c', .options.snow = opts, 
...
shap_names <- names(rc_shaps)
rc_shaps_df <- rc_shaps|>map(~flatten(.)|>as_tibble())|>
...
rc_shaps_df|>write_csv(here("Predictive_Models/RC_SHAP_vals.csv"))
  
  rc_shap_sums <- rc_shaps_df|>
...
#ggplot(aes(x = mean_SHAP, y = variable, fill = Depth_m_raw))+
  ggplot(rc_shap_sums|>filter(Depth_m_raw == 0), aes(y=reorder_within(variable, mean_SHAP, list(val_period,Depth_m_raw)), x=mean_SHAP))+
