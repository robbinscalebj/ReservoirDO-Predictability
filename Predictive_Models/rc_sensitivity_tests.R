...
randfor_best <- readRDS(here("Predictive_Models/trained_workflows/randfor_mod_lag0.Rds")) |> 
...
splits_df <- tibble(split_percent = c(rep(split_percents, 5),1))|>
...
rf_rmse_results_trans|>write_csv(here("Predictive_Models/RC_training_data_size_sensitivity_results.csv"))
rf_rmse_results_trans<-read_csv(here("Predictive_Models/RC_training_data_size_sensitivity_results.csv"))
...
training_data_og <- lag0_df|>ungroup()|>filter(validation_period == "training") 
...
training_data_sw <- lag0_df|>ungroup()|>filter(validation_period == "testing") # Want to test train/test split id sensitivity
...
rf_predictions_sw1|>write_csv(here("Predictive_Models/RC_split_switch_sensitivity_preds.csv"))
...
predictions_c|>write_csv(here("Predictive_Models/RC_split_switch_sensitivity_results.csv"))

#################################### TEST 3 ######################################
# Downsampling our training set to match the limitations in the Fayetteville data
# Fayetteville could only be sampled from 19:00 to 05:00 due to electrical issue - RC data are in UTC so have to exclude corresponding +6 hour

# Generate original train/test evaluation
training_data_ds <- lag0_df|>ungroup()|>filter(validation_period == "training")|>
  filter(hour_of_day %in% seq(2,11, by = 1))

training_recipe_ds<-recipe(DO_mg.L ~ ., data = training_data_ds)|>
  step_select(-validation_period, -val_period,-Datetime,skip = TRUE)|>
  step_normalize(all_numeric(), -all_outcomes())

## set up workflow
rf_wflow_ds <- workflow()|>
  add_recipe(training_recipe_ds)|>
  add_model(rf_model)

## fit, predict, and evaluate
rf_fit_ds <- parsnip::fit(rf_wflow_ds, training_data_ds)

testing_data_ds <- lag0_df|>ungroup()|>filter(validation_period == "testing")
rf_predictions_ds <- predict(rf_fit_ds, testing_data_ds)|>bind_cols(testing_data_ds)|>
  group_by(Depth_m)|>
  #group_by(doy)|>summarize(.pred = mean(.pred), DO_mg.L = mean(DO_mg.L))|>select(.pred, DO_mg.L)|>
  rmse(truth = DO_mg.L, estimate = .pred)|>
  rename(test_rmse_from_downsampled_training = ".estimate")|>
  select(-.metric,-.estimator)


predictions_ds_c <- left_join(rf_predictions_og|>rename(test_rmse_from_full_training_data = "rmse_original_split"),rf_predictions_ds)

predictions_ds_c|>write_csv(here("Predictive_Models/RC_downsampling_sensitivity_results.csv"))
